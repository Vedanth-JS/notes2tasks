...
from src.database.db import save_run, get_runs_for_user

from src.evaluation.analysis import analysis_to_markdown, generate_analysis
from src.evaluation.evaluation import EvalResult, run_evaluation
    ...
    with st.spinner("Running pipeline…"):
        try:
            ...
            if result.run_id and result.items:
                ...
                save_run(_DEFAULT_USER, result.run_id, str(date.today()), raw, tasks_dicts, result.analysis)
...
with tabs[0]:
    ...
    runs = get_runs_for_user(_DEFAULT_USER)
    if not runs:
